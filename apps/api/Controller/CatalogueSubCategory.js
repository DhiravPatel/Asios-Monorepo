...
async function AddCatalogueSubCategory(req, res) {
  try {
    ...
    res.status(201).json({ message: 'Catalogue sub-category Added', data: result })
  } catch (error) {
    res.status(500).json({ message: 'Error adding catalogue sub-category', error: error.message })
  }
}

async function GetAllCatalogueSubCategory(req, res) {
  try {
    const { cataloguecategory } = req.query
    const filter = {}
    ...
    res.status(500).json({ message: 'Error fetching catalogue category', error: error.message })
  }
}
